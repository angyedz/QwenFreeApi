...
function candidates() {
  const all = [...accounts.values()].filter(isAvailable);
  if (!all.length) return [];
  const active = current();
  const ordered = active && isAvailable(active) ? [active, ...all.filter((a) => a.id !== active.id)] : all;
  return ordered;
}
...
function markFailure(account, error, permanent = false) {
  ...
  const backoff = permanent ? 30 * 60 * 1000 : Math.min(15 * 60 * 1000, 30 * 1000 * 2 ** Math.min(account.failures - 1, 5));
...
module.exports = { save, load, list, get, current, candidates, select, reset, markSuccess, markFailure, isAccount, buildCookieHeader };
