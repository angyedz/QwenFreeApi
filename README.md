...
После входа токен и cookies сохраняются локально в `config/accounts/<id>.json`. Эти файлы игнорируются Git и не должны публиковаться.

## Несколько аккаунтов и autorouting

Проект поддерживает пул аккаунтов Qwen. Каждый аккаунт хранится отдельно, а сервер автоматически пробует следующий доступный аккаунт, если текущий вернул `401`, `403`, WAF challenge, `504` или сетевую ошибку.
...
Аккаунт помечается недоступным с exponential backoff после ошибки. WAF и ошибки авторизации блокируют его на более длительный период. После успешного запроса счётчик ошибок сбрасывается. Если все аккаунты недоступны, API возвращает ошибку с предложением выполнить `npm run accounts` и перелогинить аккаунт.
...
### Рекомендация для кодинг-агентов (порог сброса)
Для OpenCode и других IDE-агентов установите порог сброса контекста **~80k токенов**, чтобы IDE сама отправляла compaction. Сервер корректно обработает его (rollover на свежий тред, без дублирования истории O(n²) — см. `src/chat-adapter.js` `currentOnly`) и сохранит контекст благодаря `getHistoryBudget` (800k для 1M-моделей).
...
Autorouting временно исключит проблемный аккаунт. Откройте `npm run accounts` и перелогиньте его через `r`, либо выполните `npm run login -- --account <id>`.
