...
(async () => {
  ...
  // --- B. Existing thread + tool turn: ONLY the current message ---
  ...
  assert.ok(!contContent.includes('# Conversation history'), 'B1: no history re-embedding on existing thread');
  assert.ok(!contContent.includes('прочитай file.txt'), 'B2: earlier user message is not re-sent');
  ...
  console.log('PASS B: existing thread sends only the current message (no duplication)');
  ...
  assert.ok(!roundTwo.includes('# Conversation history'), 'C4: round 2 does not duplicate history');
